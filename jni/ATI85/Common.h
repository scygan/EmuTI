...
void TI85RefreshScreen(Image *Img)
{
  ...
  else
  {
    ...
    for (Y = 0; Y < 64; Y++) {
      for (X = 0; X < 128; X++) {
        Mask = 0x80 >> (X & 7);
        P = (pixel *)Img->Data + X*3 + Y*Img->L*3;
        T = SCREEN_BUFFER + (X / 8) + (Y * 16);
        P[0] = P[1] = P[2] = XPal[*T & Mask ? 1:0];
      }

      /* Copy just drawn line twice */
      P = (pixel *)Img->Data + Y*Img->L*3;
      memcpy(P+Img->L, P, 3*128*sizeof(pixel));
      memcpy(P+2*Img->L, P, 3*128*sizeof(pixel));
    }
#else
...
//  register int Z;
  ...
  else
  {
    ...
    for (Y = 0; Y < 64; Y++) {
      for (X = 0; X < 96; X++) {
        Mask = 0x80 >> (X & 7);
        P = (pixel *)Img->Data + X*4 + Y*Img->L*3;
      ...
      /* Copy just drawn line twice */
      P = (pixel *)Img->Data + Y*Img->L*3;
      memcpy(P+Img->L, P, 4*96*sizeof(pixel));
      memcpy(P+2*Img->L, P, 4*96*sizeof(pixel));
    }
#else
